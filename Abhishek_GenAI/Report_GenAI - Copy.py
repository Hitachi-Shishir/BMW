...
def extract_sql_query(generative_output):
    ...
    if match:
        ...
        print("No valid SQL query found in the output.")
        return None
...
def generate_report(data):
    try:
        ...
        # Assume you have this data available (you can extract it from your DataFrame as well)
        departmental_avg_time = data_frame.groupby('Department')['ResolutionTime'].mean().reset_index()

        # Adding a heading for the Departmental Resolution Time Comparison Table
        ...
        # Adding the average resolution time data to the table
        for index, row in departmental_avg_time.iterrows():
            row_cells = comparison_table.add_row().cells
            row_cells[0].text = row['Department']                # Department name
            row_cells[1].text = f"{row['ResolutionTime']:.2f}"  # Average resolution time formatted to 2 decimal places
        ...
        print("hellllllllllo")
        hdr_cells[1].text = 'Open Ticket'
        hdr_cells[2].text = 'Closed Tickets'
        print("hellllllllllo")
        for _, row in priority_summary.iterrows():
            row_cells = priority_table.add_row().cells
            row_cells[0].text = row['Priority']  # Ensure this column exists

            # Check if 'Open Tickets' is a valid column
            if 'Open Tickets' in row:
                row_cells[1].text = str(row['Open Tickets'])
            else:
                row_cells[1].text = '0'  # Default to 0 if not found

            # Calculate closed tickets safely
        ...
        doc.add_paragraph("Highlight key trends and observations based on the data, identifying areas of strength and opportunities for improvement.")
        
        # Conclusion and Recommendations
        doc.add_heading('Conclusion and Recommendations', level=2)
        doc.add_paragraph("Conclude with actionable recommendations to enhance the service desk's efficiency.")


        print(doc)
        # Save the document
        report_file = 'Service_Desk_Report.docx'
...
def save_report_locally(report_content, user_input):
    ...
    doc = Document()
    doc.add_heading(f'Report File: {user_input}', level=1)
    doc.add_page_break()
    ...
    # Add each section as a heading and content
            if section.strip():
                ...
                lines = section.split('\n')
                section_title = lines[0].strip()  # First line is the title
                ...
                # Add body
                run = doc.add_paragraph(section_body)
    ...
    if query:
        ...
        if isinstance(data, pd.DataFrame) and not data.empty:
            ...
            if report_content:
                ...
                print(f"Report generated successfully and saved at {report_filename}")
