...
public partial class Dashboard_SDDepartmentCallDetails : System.Web.UI.Page
{
    ...
    private void FillOpenClosedStatusPie()
    {
        try
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand("SD_SDDepartmentCallDashboard", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
    ...
    {
        var chartData = new ChartDataDept
        {
            DepartmentTotalTickets = dt.AsEnumerable().Select(row => row.Field<int>("Ticket Count")).ToArray(),
            Department = dt.AsEnumerable().Select(row => row.Field<string>("Status")).ToArray()
        };
        var serializer = new JavaScriptSerializer();
        string TicketCntJson = serializer.Serialize(chartData.DepartmentTotalTickets);
        string StatusJson = serializer.Serialize(chartData.Department);
    ...
    private void FillCallAging()
    {
        try
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SD_SDDepartmentCallAgingDashboard", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ServiceDesk", DropDesks.SelectedItem.ToString());
                    cmd.Parameters.AddWithValue("@OrgId", Session["SD_OrgID"].ToString());
    ...
    private void FillAgingDetails()
    {
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ServiceDesk", DropDesks.SelectedItem.Text.ToString());
                    cmd.Parameters.AddWithValue("@FROM", formattedFromDate);
                    cmd.Parameters.AddWithValue("@TO", formattedToDate);
                    cmd.Parameters.AddWithValue("@OrgId", Session["SD_OrgID"].ToString());
    ...
    protected void ChartCallsStatus_Click(object sender, ImageMapEventArgs e)
    {
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                DataTable dt = new DataTable();
                con.Open();
                string cmdstr = "SD_SDDepartmentCallDashboard";
...
public class ChartDataDept
{
    public string[] Department { get; set; }
    public int[] DepartmentTotalTickets { get; set; }
