...
var ImageDialog = /*#__PURE__*/function () {
  ...
  ImageDialog_createClass(ImageDialog, [{
    ...
    value: function initialize() {
      ...
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
        var body = ['<div class="form-group note-form-group note-group-select-from-files mt-2">', '<label for="note-dialog-image-file-' + this.options.id + '" class="note-form-label form-label">' + this.lang.image.selectFromFiles + '</label>', '<input id="note-dialog-image-file-' + this.options.id + '" class="note-image-input form-control-file note-form-control note-input form-control form-control-sm " ', ' type="file" name="files" accept="' + this.options.acceptImageFileTypes + '" multiple="multiple"/>', imageLimitation, '</div>', '<div class="form-group mt-2 note-group-image-url">', '<label for="note-dialog-image-url-' + this.options.id + '" class="note-form-label form-label">' + this.lang.image.url + '</label>', '<input id="note-dialog-image-url-' + this.options.id + '" class="note-image-url form-control form-control-sm note-form-control note-input" type="text"/>', '</div>'].join('');
      var buttonClass = 'btn btn-sm btn-grd-info note-btn note-btn-primary note-image-btn border-0';
...
var VideoDialog = /*#__PURE__*/function () {
  ...
  VideoDialog_createClass(VideoDialog, [{
    ...
    value: function initialize() {
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
      var body = ['<div class="form-group note-form-group row-fluid mt-2">', "<label for=\"note-dialog-video-url-".concat(this.options.id, "\" class=\"note-form-label form-label\">").concat(this.lang.video.url, " <small class=\"opacity-50\">").concat(this.lang.video.providers, "</small></label>"), "<input id=\"note-dialog-video-url-".concat(this.options.id, "\" class=\"note-video-url form-control form-control-sm note-form-control note-input\" type=\"text\"/>"), '</div>'].join('');
      var buttonClass = 'btn btn-grd-info btn-sm note-btn note-btn-primary note-video-btn border-0';
...
var dialog = renderer.create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"></div>', function ($node, options) {
  ...
  $node.attr({
    ...
    $node.html(['<div class="modal-dialog modal-dialog-scrollable">', '<div class="modal-content">', options.title ? '<div class="modal-header  py-2">' + '<h5 class="modal-title">' + options.title + '</h5>' + '<button type="button" class="btn rounded btn-grd-danger btn-sm" data-bs-dismiss="modal">x</button>' + '</div>' : '', '<div class="modal-body ">' + options.body + '</div>', options.footer ? '<div class="modal-footer">' + options.footer + '</div>' : '', '</div>', '</div>'].join(''));
});
