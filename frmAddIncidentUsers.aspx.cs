...
public partial class frmAddIncidentUsers : System.Web.UI.Page
{
    Random r = new Random();
        ...
        try
        {
            ...
            if (Session["SD_OrgID"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    try
                    {
                        ddlRequestType.Items.FindByText(Request.QueryString["Desk"].ToString().Replace("%20", " ")).Selected = true;
    ...
    protected void ddlRequestType_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
          ...
          else  if (ddlRequestType.SelectedItem.ToString().ToLower() == "service request")
            {
                FillHODEmail();
                showChangeControl.Visible = false;
                ...
                using (SqlTransaction transaction = con.BeginTransaction())
                {
                    using (SqlCommand cmd = new SqlCommand("SD_spSDIncident", con))
                    {
                        ...
                        if (res >= 1 && string.IsNullOrEmpty(ErrorChk))
                        {
                            ...
                            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowSuccess", "alert('" + HttpUtility.JavaScriptStringEncode(msg) + "');window.location.href='" + Request.RawUrl + "';", true);
...
, " " + Request.Url.ToString() + "Got Exception" + "Line Number :" + line.ToString() + ex.ToString());
    ...
    protected void ddlSeverity_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            if (ddlSeverity.SelectedItem.Text=="S2-High")
            {
                priority = "High";
            }
            else if (ddlSeverity.SelectedItem.Text=="S3-Medium")
            {
                priority = "Medium";
            }
            else if (ddlSeverity.SelectedItem.Text=="S4-Low")
            {
                priority = "Low";
            }
            else if (ddlSeverity.SelectedItem.Text=="S1-Very High")
            {
		priority = "Very High";
            }
    ...
    protected void ddlPriority_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            if (ddlPriority.SelectedItem.Text== "High")
            {
                Severity = "S2";
            }
            ...
            else if (ddlPriority.SelectedItem.Text== "Very High")
            {
		Severity = "S1";
            }
                            ...
                            if (!string.IsNullOrEmpty(Convert.ToString(dt.Rows[i]["fileName"])))
                            {
                                ...
                                try
                                {
                                    if (File.Exists(sourceFile))
                                    {
                                        ...
                                        cmd.Parameters.AddWithValue("@Ticketref", Ticketref);
                                        cmd.Parameters.AddWithValue("@Filepath", "/TicketAttachment/" + Ticketref + "/" + Convert.ToString(dt.Rows[i]["fileName"]));
    ...
    protected void lnkBack_Click(object sender, EventArgs e)
    {
        ...
        {
            string desk = "frmMyTickets.aspx?&redirected=true&Desk="+Request.QueryString["Desk"].ToString();
            Response.Redirect(desk);
            //Response.Write("<script type='text/javascript'>");
            //Response.Write("window.open('/frmMyTickets.aspx?&redirected=true&Desk=" + desk + "','_blank');");
    ...
    private void LoadAssets(string EmpCode)
    {
        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
        {
            string query = @"select SerialNo,Asset_Tag_ID,CategoryID,Warranty_Expiration_Date,ManufacturerID,ModelCode from ALM_AssetInventory
                            where Assigned_to_person_EMPCode='M6730' and AssetTypeCode='IT-Asset'";
