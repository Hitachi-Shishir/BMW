...
public partial class frmBulkUpdate : System.Web.UI.Page
{
    ...
    private void FillStatus()
    {
        ...
        try
        {
            DataTable SD_Priority = new SDTemplateFileds().FillStatusMerge(Request.QueryString["Desk"].ToString().Replace("%20", " "), Convert.ToString(Session["SD_OrgID"]));
    ...
    private void FillCategory1()
    {
        try
        {
           ...
           Categoryref FROM [dbo].fnGetCategoryFullPathForDesk('" + (Request.QueryString["Desk"].ToString().Replace("%20", " ")) + "'," + Convert.ToString(Session["SD_OrgID"]) + ", 1) where Level=1 order by Categoryref asc";
                con.Open();
    ...
    private DataTable FillCategoryLevel(string category, int categoryLevel)
    {
        try
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                string sql = @"	select Categoryref,categorycoderef from (select a.Categoryref as sdCategoryFK,b.Categoryref,b.categorycoderef from 
dbo.fnGetCategoryFullPathForPartition(1," + Convert.ToString(Session["SD_OrgID"]) + ") a  " +
"left join  dbo.fnGetCategoryFullPathForPartition(1," + Convert.ToString(Session["SD_OrgID"]) + ") b " +
"on a.id=b.sdCategoryFK) c where c.sdCategoryFK = '" + category + "' and c.Categoryref != '' " +
"order by categorycoderef asc";
                con.Open();
                using (SqlCommand cmd = new SqlCommand(sql, con))
    ...
    private void FillResolutionDetails()
    {
        ...
        try
        {
            ...
            DataTable SD_Resolution = new FillSDFields().FillResolution(Convert.ToString(Session["OrgID"])); 

            if (SD_Resolution.Rows.Count > 0 && SD_Resolution != null)
            {
