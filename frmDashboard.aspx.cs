...
public partial class frmDashboard : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            string sql = "SELECT FileData FROM SD_User_Master WITH(NOLOCK) WHERE FileData IS NOT NULL AND LoginName = '" + Convert.ToString(Session["LoginName"]) + "'";
...
";
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (todate > DateTime.ParseExact(DateTime.Now.ToString("dd-MM-yyyy"), "dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture))
            {
                return;
            }
